...
- **섹션 앵커 네비게이션** (IntersectionObserver 기반 스크롤 스파이)
  - Sticky 네비게이션 (`top-20`, `h-14`, 중앙 정렬)
  - 활성 섹션 자동 감지 (뷰포트 50% 기준)
  - 활성 버튼: `bg-green-600 text-white`
  - 비활성 버튼: `text-gray-500 hover:bg-gray-100`
- **8개 섹션**:
  1. LAKE 브랜드 이미지
  2. 미션 (2개 문단으로 분리, `space-y-5`)
  3. 핵심가치 (4개 카드)
  4. 4대 단절 (4개 카드, `max-w-5xl` 너비 제한, 테두리 `border-{color}-200`)
  5. 사업 영역 (4개 카드, `max-w-6xl` 너비 제한, Bio-Tech → Content → Product → People)
  6. 팀 (3명)
  7. 연혁 (타임라인, PC에서 우측 이동 `md:ml-[calc(50%-260px)]`)
  8. 회사정보 (대표: 박충신, 주소: 강원특별자치도 속초시 관광로 115)
- **밝은 히어로 섹션**: `bg-gradient-to-b from-green-100/70 to-white`
- **스크롤 마진**: 모든 섹션 `scroll-mt-36` (헤더 80px + 네비 56px)
...
3. **전국 17개 시도 SVG 지도 기반 길 안내**
   - `components/maps/*.tsx` (17개 파일)
   - 오디오가 등록된 지역만 파란색 표시
   - 3단계 선택: 대한민국 → 도 → 시군구 → 길
...
**감정 기록 플로우**:
1. 비로그인: "감정 기록" 클릭 → 로그인 모달
2. 로그인 + 오늘 미기록: 5단계 감정 기록 모달
3. 로그인 + 오늘 이미 기록: "이미 기록됨" 모달 + 마이페이지 바로가기
...
|------|----------|---------------|--------|
| 전체 페이지 | 로그인 UI | 표시 | 표시 |
| 주간 보고서 버튼 | - | "주간 감정 보고서" | "주간 감정 보고서" + "이번 주 보고서 (테스트)" |
...
- 로그아웃
- PC/모바일 분리 레이아웃
- **디자인 테마**: 그린-에메랄드 그라데이션 (마이페이지와 통일)
- **배경색**: `#f5f1e8` (어스 베이지)
- **Toast 알림** (`showToast()`)
...
4. **3단계 지도 선택**
   1. 대한민국 전체 지도 (`KoreaMap.tsx`)
   2. 도 선택 시 → 해당 도 상세 지도 (예: `GangwonMapWithAudio.tsx`)
   3. 시군구 선택 시 → 길 목록 표시
5. **오디오가 등록된 지역만 파란색 표시**
