...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (!result.success || !result.post) {
    return {
      title: '게시글을 찾을 수 없습니다 | 공지사항 | WithTheLake',
    }
  }

  return {
    title: `${result.post.title} | 공지사항 | WithTheLake`,
...
export default async function NoticePostPage({ params }: PageProps) {
  const { id } = await params
  const [result, adjacentResult, currentUserId, adminStatus] = await Promise.all([
    getPost(id),
    getAdjacentPosts(id, 'notice'),
    getCurrentUserId(),
    checkIsAdmin()
  ])
